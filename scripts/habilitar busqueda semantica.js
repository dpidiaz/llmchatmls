...
const BLOCK=[
  ...
  '  let semanticManifestTask=null;',
  "  async function loadSemanticManifest(){",
  '    if(semanticManifestTask)return semanticManifestTask;',
  "    semanticManifestTask=fetch('data/semantic/manifest.json',{cache:'default'}).then(async response=>{",
  "      if(response.status===404)return null;",
  "      if(!response.ok)throw new Error('Manifest semántico no disponible');",
  '      const manifest=await response.json();',
  "      if(manifest?.standard!=='MLS R32'||manifest?.promptVersion!=='32.0'||manifest?.version!=='1.0'||manifest?.model!=='@cf/baai/bge-m3'||!manifest?.languages)throw new Error('Manifest semántico incompatible');",
  '      return manifest;',
  '    }).catch(error=>{semanticManifestTask=null;throw error});',
  '    return semanticManifestTask;',
  '  }',
  "  async function loadSemanticIndex(slug,manifest){",
  "    if(!manifest?.languages?.[slug])throw new Error('Idioma sin índice semántico publicado: '+slug);",
  ...
  "      if(meta?.standard!=='MLS R32'||meta?.promptVersion!=='32.0'||meta?.version!=='1.0'||meta?.model!=='@cf/baai/bge-m3'||meta?.language!==slug||meta?.corpusBuildId!==manifest.corpusBuildId)throw new Error('Metadata semántica inválida: '+slug);",
  ...
  '    }));',
  '    try{',
  '      const manifest=await loadSemanticManifest();',
  '      const semanticSlugs=manifest?slugs.filter(slug=>manifest.languages?.[slug]):[];',
  '      if(semanticSlugs.length){',
  '        const queryVector=await semanticQueryVector(q);',
  '        await Promise.all(semanticSlugs.map(async slug=>{',
  '          try{',
  '            const semantic=await loadSemanticIndex(slug,manifest);',
  ...
  '        }));',
  '      }',
  '    }catch(error){console.warn("MLS semantic manifest fallback",error)}',
