...
const BACKEND_BLOCK=[
  ...
  'const MLS_TRANSLATOR_PROVIDER_ID = "cloudflare-translator";',
  'const MLS_TRANSLATOR_ESTIMATED_NEURONS = 40;',
  'const MLS_TRANSLATOR_FREE_DAILY_NEURONS = 10000;',
  ...
  '}',
  'function mlsTranslatorRound(value, digits = 2) {',
  '  const factor = 10 ** digits;',
  '  return Math.round((Number(value) || 0) * factor) / factor;',
  '}',
  'function mlsTranslatorUsagePayload(promptTokens, completionTokens, neurons, measured, budget) {',
  '  const dailyNeurons = Number(budget?.dailyNeurons || 0);',
  '  const dailyReservedNeurons = Number(budget?.dailyReservedNeurons || 0);',
  '  const targetNeurons = Number(budget?.targetNeurons || WIKI_CLOUDFLARE_NEURON_TARGET || 9000);',
  '  return {',
  '    provider: MLS_TRANSLATOR_PROVIDER_ID,',
  '    promptTokens: Math.max(0, Number(promptTokens) || 0),',
  '    completionTokens: Math.max(0, Number(completionTokens) || 0),',
  '    neurons: mlsTranslatorRound(neurons, 4),',
  '    measured: measured === true,',
  '    dailyNeurons: mlsTranslatorRound(dailyNeurons, 2),',
  '    dailyReservedNeurons: mlsTranslatorRound(dailyReservedNeurons, 2),',
  '    targetNeurons,',
  '    freeDailyAllocationNeurons: MLS_TRANSLATOR_FREE_DAILY_NEURONS,',
  '    remainingTargetNeurons: mlsTranslatorRound(Math.max(0, targetNeurons - dailyNeurons - dailyReservedNeurons), 2)',
  '  };',
  '}',
  'async function mlsTranslatorBudgetSnapshot(env) {',
  '  try { return await wikiStore(env).getCloudflareBudget(); } catch { return { dailyNeurons: 0, dailyReservedNeurons: 0, targetNeurons: WIKI_CLOUDFLARE_NEURON_TARGET }; }',
  '}',
  ...
  '      const budget = await mlsTranslatorBudgetSnapshot(env);',
  '      return Response.json({ ok: true, usedAi: false, sameLanguage: true, sourceLanguage, targetLanguage, translation: text, usage: mlsTranslatorUsagePayload(0, 0, 0, true, budget) }, { headers: { "cache-control": "no-store" } });',
  '    }',
  ...
  '    const reservation = await wikiStore(env).reserveCloudflareBudget(MLS_TRANSLATOR_ESTIMATED_NEURONS);',
  '    if (!reservation.ok) {',
  '      const retryAfter = reservation.circuitOpen && reservation.circuit?.resetAt ? Math.max(60, Math.ceil((Date.parse(reservation.circuit.resetAt) - Date.now()) / 1000)) : secondsUntilNextUtcDay();',
  '      const budget = await mlsTranslatorBudgetSnapshot(env);',
  '      const message = reservation.circuitOpen ? "Workers AI FREE agotado hasta 00:00 UTC. La pronunciación local sigue disponible." : "MLS alcanzó su presupuesto protegido de Workers AI por hoy. La pronunciación local sigue disponible.";',
  '      return Response.json({ ok: false, aiUnavailable: true, quotaProtected: true, circuitOpen: reservation.circuitOpen === true, error: message, usage: mlsTranslatorUsagePayload(0, 0, 0, true, budget) }, { status: 429, headers: { "cache-control": "no-store", "retry-after": String(retryAfter) } });',
  '    }',
  '    const reserved = Number(reservation.reserved || MLS_TRANSLATOR_ESTIMATED_NEURONS);',
  '    const system = [',
  ...
  '      measured = promptTokens + completionTokens > 0;',
  '      neurons = measured ? cloudflareNeurons(MLS_TRANSLATOR_MODEL_ID, promptTokens, completionTokens) : reserved;',
  '      budgetAfter = await wikiStore(env).settleCloudflareBudget(reserved, neurons);',
  '      settled = true;',
  '      const parsed = mlsTranslatorParseJson(mlsTranslatorModelText(result));',
  '      const translation = mlsTranslatorText(parsed?.translation, 2400);',
  '      if (!translation) throw new Error("No fue posible obtener una traducción válida.");',
  '      await recordProviderUsage(env, MLS_TRANSLATOR_PROVIDER_ID, promptTokens, completionTokens, false).catch(error => console.warn("MLS Translator usage log unavailable:", error instanceof Error ? error.message : String(error)));',
  '      return Response.json({ ok: true, usedAi: true, model: MLS_TRANSLATOR_MODEL_ID, sourceLanguage, targetLanguage, translation, usage: mlsTranslatorUsagePayload(promptTokens, completionTokens, neurons, measured, budgetAfter) }, { headers: { "cache-control": "no-store", "x-content-type-options": "nosniff" } });',
  '    } catch (error) {',
  '      if (!settled) await wikiStore(env).releaseCloudflareBudget(reserved).catch(() => {});',
  '      const message = wikiErrorMessage(error);',
  '      const kind = workersAiFailureKind(message);',
  '      await recordProviderUsage(env, MLS_TRANSLATOR_PROVIDER_ID, promptTokens, completionTokens, true).catch(() => {});',
  '      if (kind === "quota") {',
  '        await wikiStore(env).markQuotaExhausted("translator", message).catch(() => {});',
  '        const budget = await mlsTranslatorBudgetSnapshot(env);',
  '        return Response.json({ ok: false, aiUnavailable: true, quotaProtected: true, circuitOpen: true, error: "Workers AI FREE agotado hasta 00:00 UTC. La pronunciación local sigue disponible.", usage: mlsTranslatorUsagePayload(promptTokens, completionTokens, neurons, measured, budget) }, { status: 429, headers: { "cache-control": "no-store", "retry-after": String(secondsUntilNextUtcDay()) } });',
  '      }',
  '      if (kind === "paid") return Response.json({ ok: false, aiUnavailable: true, zeroCostPolicy: true, error: "El modelo requeriría Workers Paid. MLS lo bloqueó para mantener costo $0.00." }, { status: 503, headers: { "cache-control": "no-store" } });',
  '      console.warn("MLS Translator AI unavailable:", message);',
  '      const budget = budgetAfter || await mlsTranslatorBudgetSnapshot(env);',
  '      return Response.json({ ok: false, aiUnavailable: true, error: "La traducción mejorada no está disponible temporalmente. La pronunciación local sigue disponible.", usage: mlsTranslatorUsagePayload(promptTokens, completionTokens, neurons, measured, budget) }, { status: 503, headers: { "cache-control": "no-store", "retry-after": "60" } });',
  '    }',
  '  } catch (error) {',
