...
function augmentEditorialOpenApi(text) {
  ...
  const codeParam={name:'code',in:'query',required:true,schema:{type:'string',pattern:'^MLS-V\\d{2}-\\d{4}
...
if (require.main === module) main();
  ...
  const simpleResponses={'200':{description:'Operación Evidence completada.'},'400':{description:'Solicitud inválida.'},'401':{description:'Clave ausente o incorrecta.'},'404':{description:'Entrada o recurso no encontrado.'},'409':{description:'Conflicto de versión, snapshot o revisión.'},'422':{description:'Evidence o metadata no válida.'}};
  api.paths['/api/wiki/editorial/evidence/status']={get:{operationId:'estadoEvidenceMLS',summary:'Consultar estado agregado de Evidence & Provenance.', 'x-openai-isConsequential':false,responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/entry']={get:{operationId:'evidenceEntradaMLS',summary:'Obtener artículo canónico, versión exacta, estado Evidence y reviews.', 'x-openai-isConsequential':false,parameters:[codeParam],responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/sources']={get:{operationId:'fuentesEntradaMLS',summary:'Listar Sources vinculadas a una entrada y su referencia APA renderizada cuando sea válida.', 'x-openai-isConsequential':false,parameters:[codeParam],responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/validate']={post:{operationId:'validarEvidenceMLS',summary:'Validar cobertura Evidence y APA sin promover estado.', 'x-openai-isConsequential':false,requestBody:{required:true,content:{'application/json':{schema:{type:'object',properties:{code:{type:'string'}},required:['code'],additionalProperties:false}}}},responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/proposal']={post:{operationId:'proponerEvidenceMLS',summary:'Persistir una propuesta idempotente de Sources, Claims, EvidenceLinks y conflictos para la versión exacta del artículo.', 'x-openai-isConsequential':true,requestBody:{required:true,content:{'application/json':{schema:{type:'object',properties:{code:{type:'string'},articleGeneratedAt:{type:'string'},articleHash:{type:'string'},expectedEvidenceRevision:{type:'integer',minimum:0},sources:{type:'array',maxItems:30,items:{type:'object'}},claims:{type:'array',maxItems:50,items:{type:'object'}},links:{type:'array',maxItems:100,items:{type:'object'}},conflicts:{type:'array',maxItems:30,items:{type:'object'}}},required:['code','articleGeneratedAt','articleHash','expectedEvidenceRevision','sources','claims','links'],additionalProperties:false}}}},responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/verify']={post:{operationId:'verificarEvidenceMLS',summary:'Crear un verification review ligado al snapshot actual y promover a VERIFIED solo si el backend lo permite.', 'x-openai-isConsequential':true,requestBody:{required:true,content:{'application/json':{schema:{type:'object',properties:{code:{type:'string'},expectedEvidenceRevision:{type:'integer',minimum:0},reviewerType:{type:'string',enum:['chatgpt','human','system']},reviewer:{type:'string'},verificationMethod:{type:'string'},notes:{type:'string'},runId:{type:'string'}},required:['code','expectedEvidenceRevision'],additionalProperties:false}}}},responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/review']={post:{operationId:'revisarEvidenceMLS',summary:'Registrar una revisión editorial posterior sobre un snapshot VERIFIED vigente.', 'x-openai-isConsequential':true,requestBody:{required:true,content:{'application/json':{schema:{type:'object',properties:{code:{type:'string'},expectedEvidenceRevision:{type:'integer',minimum:0},reviewerType:{type:'string',enum:['chatgpt','human','system']},reviewer:{type:'string'},verificationMethod:{type:'string'},notes:{type:'string'},runId:{type:'string'}},required:['code','expectedEvidenceRevision'],additionalProperties:false}}}},responses:simpleResponses}};
  api.paths['/api/wiki/editorial/evidence/revision/propose']={post:{operationId:'proponerRevisionEvidenceMLS',summary:'Guardar una revisión de artículo propuesta sin sobrescribir wiki_articles.', 'x-openai-isConsequential':true,requestBody:{required:true,content:{'application/json':{schema:{type:'object',properties:{code:{type:'string'},expectedArticleHash:{type:'string'},articleMarkdown:{type:'string'},changeReason:{type:'string'},evidenceReviewId:{type:'string'}},required:['code','expectedArticleHash','articleMarkdown','changeReason'],additionalProperties:false}}}},responses:simpleResponses}};
  return JSON.stringify(api);
}
function buildChatRuntime(root = process.cwd()) {
  const read = file => fs.readFileSync(path.join(root, file), 'utf8');
