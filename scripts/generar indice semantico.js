...
async function embedBatch(texts,{token,accountId,model=MODEL,endpoint=process.env.MLS_EMBEDDING_ENDPOINT,endpointKey=process.env.MLS_EMBEDDING_KEY}){
  ...
  if(endpoint){
    ...
    for(let attempt=1;attempt<=8;attempt++){
      ...
      if(response.ok&&payload?.ok===true&&Array.isArray(payload?.vectors)){
...
async function generateLanguage(language,options={}){
  ...
  const endpoint=String(options.endpoint||process.env.MLS_EMBEDDING_ENDPOINT||'').trim();
  const endpointKey=String(options.endpointKey||process.env.MLS_EMBEDDING_KEY||'').trim();
  ...
  for(let start=0;start<records.length;start+=BATCH_SIZE){
    ...
    const vectors=await embedBatch(batch.map(x=>x.text),{token,accountId,endpoint,endpointKey});
...
async function main(){
  ...
  for(const language of languages){
    const result=await generateLanguage(language,{token,accountId,endpoint,endpointKey,force});
