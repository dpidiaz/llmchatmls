...
if (!backend.includes("/api/wiki/regenerate/")) {
  if (backend.includes("ON CONFLICT(code) DO NOTHING")) {
    backend = replaceOnce(
      backend,
  ...
  if (backend.includes("async function processWikiEntry(env, code, alreadyClaimed = false) {")) {
    backend = replaceOnce(
      backend,
  ...
  if (backend.includes("  if (await articleExists(env, code)) {")) {
    backend = replaceOnce(
      backend,
  ...
  if (backend.includes("  if (legacy?.articleMarkdown) {")) {
    backend = replaceOnce(
      backend,
