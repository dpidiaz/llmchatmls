...
const family=t=>String(t.title||'').includes('Verb')?'conjugacion_verbal':'pronombres_regimen';
...
test('coverage map: exact completion by language uses canonical totals',()=>{
  const x=coverage.mlsCoverageAggregate(articles,audits,langs,family);
...
test('coverage map: pending never becomes negative',()=>{
  ...
  const x=coverage.mlsCoverageAggregate(rows,[],[{slug:'ingles',name:'Inglés',total:5}],family);
...
test('coverage map: semantic audits are version exact',()=>{
  const x=coverage.mlsCoverageAggregate(articles,[...audits,{code:'ES2',article_generated_at:'old',verdict:'watch'}],langs,family);
  const es=x.exact.byLanguage.find(v=>v.language==='espanol-guatemala');
