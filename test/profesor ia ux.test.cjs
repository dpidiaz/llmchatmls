...
test('estilos cumplen legibilidad contraste claro y objetivos táctiles',()=>{const {api}=apiWithDom();const css=api.STYLE_TEXT;assert.match(css,/font-size:max\(11pt/);assert.match(css,/background:#fbfaf7/);assert.match(css,/color:#182026/);assert.match(css,/min-height:44px/);assert.match(css,/@media\(max-width:600px\)/);assert.match(css,/prefers-reduced-motion:reduce/);assert.doesNotMatch(css,/color:\s*#fff(?:fff)?\b/i);});
