...
test('También mira keeps editorial results first and caps the list at five',()=>{
  assert.match(reader,/function relatedFromMarkdown\(markdown,catalog,limit=5\)/);
  assert.match(reader,/const editorial=relatedFromMarkdown\(markdown,catalog,limit\);/);
...
test('También mira validates related build, model, language and canonical catalog membership',()=>{
  assert.match(reader,/\/data\/related\/manifest\.json/);
  assert.match(reader,/relatedManifest\?\.corpusBuildId!==manifest\.corpusBuildId/);
  assert.match(reader,/relatedManifest\?\.semanticModel!=='@cf\/baai\/bge-m3'/);
...
test('semantic related assets are optional and never become a reader dependency',()=>{
  assert.match(reader,/catch\(error\)\{\s*console\.warn\('MLS related semantic fallback',language,error\);\s*return editorial;/);
