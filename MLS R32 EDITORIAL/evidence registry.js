...
function rowToSource(row){
  ...
  return {sourceId:row.source_id,identityKind:row.identity_kind,identityKey:row.identity_key,metadataHash:row.metadata_hash,sourceType:row.source_type,authorityTier:row.authority_tier,status:row.status,title:row.title,
    authors:JSON.parse(row.authors_json||'[]'),contributors:JSON.parse(row.contributors_json||'[]'),institution:row.institution||null,publicationYear:row.publication_year??null,publicationDate:row.publication_date||null,
...
function sourceParams(s,now,createdAt=now){return [s.sourceId,s.identityKind,s.identityKey,s.metadataHash,s.sourceType,s.authorityTier,s.status,s.title,JSON.stringify(s.authors),JSON.stringify(s.contributors),dbValue(s.institution),dbValue(s.publicationYear),dbValue(s.publicationDate),dbValue(s.publisher),dbValue(s.edition),dbValue(s.containerTitle),dbValue(s.journal),dbValue(s.volume),dbValue(s.issue),dbValue(s.pages),dbValue(s.articleNumber),dbValue(s.isbn),dbValue(s.issn),dbValue(s.doi),dbValue(s.canonicalUrl),dbValue(s.language),JSON.stringify(s.topics),dbValue(s.resourceVersion),dbValue(s.supersedesSourceId),dbValue(s.accessedAt),createdAt,now];}
