...
var WikiStore = class extends DurableObject {
  ...
  async reserveCloudflareBudget(estimatedNeurons) {
    ...
    if (state.workersAiCircuit?.status === "quota_exhausted" && Date.now() < Date.parse(state.workersAiCircuit.resetAt)) {
      await this.saveState(state);
      return { ok: false, circuitOpen: true, circuit: state.workersAiCircuit };
    }
  ...
  async getCloudflareBudget() {
    ...
    return {
      ...
      , workersAiCircuit: state.workersAiCircuit
    };
  }
...
function automaticMaterializerClient() {
  ...
  // En la SPA nativa el lector ya posee un trigger idempotente por código.
  // Este cliente sigue instalando los controles de Profesor IA, sin duplicar
  // solicitudes de materialización.
  const nativeReaderOwnsMaterialization = () => Boolean(window.__MLS_NATIVE_READER_MATERIALIZER__);
    ...
    for (const delay of [80, 250, 600, 1200]) {
      setTimeout(() => {
        if (version === navigationVersion) {
          ensureAiControls();
          wireListenButtons();
          if (!nativeReaderOwnsMaterialization()) materializeCurrentEntry();
        }
      }, delay);
    }
  }
...
var WorkersQuotaExceededError = class extends NoProviderAvailableError {
  constructor(message, delaySeconds = secondsUntilNextUtcDay()) {
    super(message, delaySeconds);
    this.name = "WorkersQuotaExceededError";
  }
};
var ZeroCostPolicyError = class extends Error {
  constructor(message) { super(message); this.name = "ZeroCostPolicyError"; }
};
...
async function runCloudflareProvider(env, provider, messages, maxTokens, temperature) {
  ...
  if (!reservation.ok) {
    if (reservation.circuitOpen) throw new WorkersQuotaExceededError("Workers AI FREE agotado hasta 00:00 UTC. Runner detenido para mantener costo $0.", Math.max(60, Math.ceil((Date.parse(reservation.circuit.resetAt) - Date.now()) / 1000)));
    throw new NoProviderAvailableError("Workers AI alcanz\xF3 el presupuesto aut\xF3nomo del 90 %.", secondsUntilNextUtcDay());
  }
  ...
  try {
    ...
    for (let inferenceAttempt = 1; inferenceAttempt <= 3; inferenceAttempt++) {
      try {
        ...
        text = wikiTextResult(result);
        if (text) break;
        lastInferenceError = new Error(`${provider.model} no devolvi\xF3 texto (intento ${inferenceAttempt} de 3).`);
      } catch (error) {
        const kind = workersAiFailureKind(wikiErrorMessage(error));
        if (kind === "quota" || kind === "paid") throw error;
    ...
    if (!text || !result) throw lastInferenceError || new Error(`${provider.model} no devolvi\xF3 texto despu\xE9s de 3 intentos.`);
    ...
    if (kind === "quota") {
      ...
      throw new WorkersQuotaExceededError("Workers AI FREE agotado hasta 00:00 UTC. Runner detenido para mantener costo $0.", secondsUntilNextUtcDay());
    }
    if (kind === "paid") throw new ZeroCostPolicyError("Workers AI indicó 5035: el modelo requiere Workers Paid. Runner detenido por política ZERO COST.");
    throw error;
  }
}
...
async function runWithFallback(env, seedCode, messages, maxTokens, temperature, excludeId, preferCheapCloudflare = false) {
  ...
  for (const provider of providers) {
    try {
      ...
      lastError = error;
      if (error instanceof WorkersQuotaExceededError || error instanceof ZeroCostPolicyError) throw error;
...
function workersAiFailureKind(message) {
  const normalized = String(message || "").toLowerCase().replace(/\s+/g, " ");
  if (/\b3040\b|out of capacity|sin capacidad/.test(normalized)) return "capacity";
  if (/\b5035\b|workers paid|requires paid/.test(normalized)) return "paid";
  if (/\b3036\b|daily free allocation|10,000 neurons|10000 neurons|used up your daily/.test(normalized)) return "quota";
  if (/\bhttp\s*429\b|\b429\b/.test(normalized)) return "quota";
  return "other";
}
function isWorkersAIDailyQuotaError(message) { return workersAiFailureKind(message) === "quota"; }
